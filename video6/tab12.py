...
class FDMA_Cuboids(ThreeDScene):
    def construct(self):
        ...
        fdma_colors = [TEAL_C, MAROON_D, GREEN, YELLOW, GRAY, RED]
        ...
        schedule = [
            ...
            (1, 0.5, 6.5), # User 2 (Freq 2)
            (1, 0.5, 6.5), # User 3 (Freq 3)
            (1, 0.5, 6.5), # User 4 (Freq 4)
            (1, 0.5, 6.5), 
            (1, 0.5, 6.5),
        ]
        ...
        axes_3d.shift(DOWN * 2 + RIGHT*0.8)
        
        # Fixed labels (Static)
        labels = VGroup(
            Text("Time", color=BLUE, font_size=20).move_to(axes_3d.c2p(8, 0, 0) + RIGHT * 0.5),
            Text("Frequency", color=BLUE, font_size=20).move_to(axes_3d.c2p(0, -5, 0) + UP * 0.5),
            Text("Code", color=BLUE, font_size=20).rotate(90*DEGREES, axis=RIGHT).move_to(axes_3d.c2p(0, 0, 6.5) + UP * 0.5)
        )
        ...
        for i, (freq_idx, start, end) in enumerate(schedule):
            ...
            freq_width = 2  # Width of the frequency band
            power_height = 0.8 # Constant power
            ...
            # 1. Create label text
            c_text = MathTex(f"f_{{{i+1}}}", color=WHITE, font_size=24)
            ...
            c_text.rotate(90 * DEGREES, axis=RIGHT)
            c_text.rotate(90 * DEGREES, axis=OUT) #Adjust this if it looks sideways

            #3. Position it at the END of the block in Time --> Frequency 
            #X = end + 0.5 (slight buffer)
            #Y = freq_idx
            #Z = 0 (on the floor) or power_height/2 (floating)
            
            label_pos = axes_3d.c2p(end+0.9 , 1, i+1)
            c_text.move_to(label_pos)
            
            freq_labels.add(c_text)
        # --- ANIMATION ---
