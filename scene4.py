...
class Scene4Case1SameCell(Scene):
    def construct(self):
        ...
        case_tag = Text("CASE 1", font_size=28, color=ACCENT_BLUE, weight=BOLD)
        ...
        self.play(FadeIn(hdr, shift=DOWN*0.2), run_time=1.2, rate_func=rate_functions.ease_out_cubic)
        self.wait(0.3)

        # ── Left-Shifted Topology (Prevents Text Overlap) ──
        base_center = LEFT * 1.6 + DOWN * 0.1

        hex1 = make_hex(base_center, ACCENT_BLUE, scale=2.3)
        cell_lbl = Text("Cell 1", font_size=18, color=ACCENT_BLUE,
                        weight=BOLD).move_to(base_center + DOWN*2.6)
        ...
        msc = make_msc(base_center + RIGHT*3.3 + UP*1.4, label="MSC", col=ACCENT_BLUE)
        hlr = make_db(base_center + LEFT*2.9 + UP*1.4, "hlr_box.svg", "HLR", ACCENT_BLUE)
        vlr = make_db(base_center + LEFT*2.9 + UP*0.35, "vlr_box.svg", "VLR", ACCENT_BLUE)

        caller   = make_mobile(base_center + LEFT*1.7 + DOWN*0.5, ALT_YELLOW, "Caller")
        receiver = make_mobile(base_center + RIGHT*1.7 + DOWN*0.5, ACCENT_GRN, "Receiver")
        ...
        # ── Elegant Arrows with Text Background Masks ──
        def flash_arrow(start_mob, end_mob, col, lbl, direction=UP):
            ...
            mid = arr.get_center()
            t = Text(lbl, font_size=14, color=col, weight=BOLD).move_to(mid + direction*0.35)
            
            # Masking background behind text to prevent arrow intersections
            t_bg = SurroundingRectangle(t, color=DARK_BG, fill_opacity=0.85, stroke_width=0, buff=0.03)
        ...
        step_texts = VGroup()
        step_data = [
            "① Caller → BS (Request via RECC)",
            "② BS → MSC (Forward request)",
            "③ MSC ↔ HLR/VLR (Verify user)",
        ...
        for i, s in enumerate(step_data):
            col = [ALT_YELLOW, ALT_YELLOW, ALT_YELLOW, ALT_YELLOW, DIM_GREY, ACCENT_GRN][i]
            t = Text(s, font_size=15, color=col, weight=BOLD)
            step_texts.add(t)
        ...
        self.play(Create(a1[0]), FadeIn(VGroup(a1[1], a1[2])), FadeIn(step_texts[0]), 
                  run_time=0.8, rate_func=rate_functions.ease_out_cubic)
        self.wait(2.0)

        # Step 2
        a2 = flash_arrow(bs, msc, ALT_YELLOW, "Forward", direction=UP+LEFT) # Shifted UP+LEFT
        self.play(Create(a2[0]), FadeIn(VGroup(a2[1], a2[2])), FadeIn(step_texts[1]), 
                  run_time=0.8, rate_func=rate_functions.ease_out_cubic)
        self.wait(2.0)

        # Step 3
        a3 = Arrow(msc.get_center(), hlr.get_center(), buff=0.4, color=ALT_YELLOW,
                   stroke_width=3.5, max_tip_length_to_length_ratio=0.15)
        a3b= Arrow(hlr.get_center(), msc.get_center(), buff=0.4, color=ALT_YELLOW,
                   stroke_width=3.0, max_tip_length_to_length_ratio=0.15).shift(DOWN*0.1)
                   
        verify_lbl = Text("Verify", font_size=14, color=ALT_YELLOW, weight=BOLD).move_to(a3.get_center() + UP*0.35)
        v_bg = SurroundingRectangle(verify_lbl, color=DARK_BG, fill_opacity=0.85, stroke_width=0, buff=0.03)
        lbl_grp = VGroup(v_bg, verify_lbl)
        
        steps_grp.add(VGroup(a3, a3b, lbl_grp))
        self.play(Create(a3), Create(a3b), FadeIn(lbl_grp), FadeIn(step_texts[2]), 
                  run_time=1.0, rate_func=rate_functions.ease_out_cubic)
        self.wait(2.0)
                  
        # Step 4
        a4 = flash_arrow(msc, receiver, ALT_YELLOW, "Page", direction=UP+RIGHT)
        self.play(Create(a4[0]), FadeIn(VGroup(a4[1], a4[2])), FadeIn(step_texts[3]), 
                  run_time=0.8, rate_func=rate_functions.ease_out_cubic)
        self.wait(2.0)
        ...
        tch_lbl = Text("TCH", font_size=14, color=DIM_GREY, weight=BOLD).move_to(tch.get_center() + DOWN*0.35)
        tch_bg = SurroundingRectangle(tch_lbl, color=DARK_BG, fill_opacity=0.85, stroke_width=0, buff=0.03)
        tch_grp = VGroup(tch_bg, tch_lbl)
        
        steps_grp.add(VGroup(tch, tch_grp))
        self.play(Create(tch), FadeIn(tch_grp), FadeIn(step_texts[4]), 
                  run_time=0.8, rate_func=rate_functions.ease_out_cubic)
        self.wait(2.0)

        # Step 6 - Finale Subtle Dashed Box
        f_box = SurroundingRectangle(VGroup(caller, receiver, bs), color=DIM_GREY,
                                     corner_radius=0.25, stroke_width=2, buff=0.5)
        dashed_box = DashedVMobject(f_box, num_dashes=30, dashed_ratio=0.5).set_opacity(0.4)
        
        self.play(Create(dashed_box), FadeIn(step_texts[5]), 
        ...
        self.play(FadeOut(all_obj, shift=UP*0.3), run_time=1.0, rate_func=rate_functions.ease_in_out_sine)
        self.wait(0.2)
