...
class Scene5Case2DiffCellSameMSC(Scene):
    def construct(self):
        ...
        # ── Topology Shifted Right (Prevents Text Overlap) ──
        ...
        lbl1 = Text("Cell 1", font_size=16, color=ACCENT_BLUE, weight=BOLD).move_to(c1_center + DOWN*2.2)
        lbl2 = Text("Cell 2", font_size=16, color=ACCENT_BLUE, weight=BOLD).move_to(c2_center + DOWN*2.2)
        ...
        vlr = make_db(RIGHT*mid_x + RIGHT*2.2 + UP*1.8, "vlr_box.svg", "VLR", ACCENT_BLUE, scale=0.26)
        ...
        self.play(
            LaggedStart(
                ...
                FadeIn(msc, scale=0.8), FadeIn(vlr, scale=0.8), FadeIn(hlr, scale=0.8),
        ...
        step_data = [
            ...
            ("② MSC queries VLR\n    → receiver in Cell 2", ALT_YELLOW),
            ("③ Paging → BS₂ → Receiver", ALT_YELLOW),
            ("④ Channel allocated\n    in both cells", DIM_GREY),
            ("⑤ Call connected\n    via MSC ✓", ACCENT_GRN),
        ]
        step_texts = VGroup(*[Text(s, font_size=15, color=c, weight=BOLD) for s, c in step_data])
        step_texts.arrange(DOWN, buff=0.3, aligned_edge=LEFT).to_edge(LEFT, buff=0.45).shift(DOWN*0.5)
        ...
        self.play(Create(a1), Create(a1b), FadeIn(step_texts[0], shift=RIGHT*0.2), 
                  run_time=0.8, rate_func=rate_functions.ease_out_cubic)
        self.wait(2.0)

        # Step 2
        vlr_highlight = SurroundingRectangle(vlr, color=ACCENT_BLUE, stroke_width=3.5, buff=0.2, corner_radius=0.15)
        vlr_glow = vlr_highlight.copy().set_stroke(width=6, opacity=0.3).scale(1.05)
        vlr_grp = VGroup(vlr_glow, vlr_highlight)
        
        self.play(Create(vlr_highlight), FadeIn(vlr_glow), FadeIn(step_texts[1], shift=RIGHT*0.2), 
                  run_time=0.8, rate_func=rate_functions.ease_out_cubic)
        self.wait(2.0)

        # Step 3
        a3 = Arrow(msc.get_bottom() + RIGHT*0.2, bs2.get_top(), buff=0.25, color=ALT_YELLOW,
                   stroke_width=3.5, max_tip_length_to_length_ratio=0.15)
        a3b= Arrow(bs2.get_center(), receiver.get_top(), buff=0.3, color=ALT_YELLOW,
                   stroke_width=3.5, max_tip_length_to_length_ratio=0.15)
        arrows_grp.add(a3, a3b)
        self.play(Create(a3), Create(a3b), FadeIn(step_texts[2], shift=RIGHT*0.2), 
                  run_time=0.8, rate_func=rate_functions.ease_out_cubic)
        self.wait(2.0)
        ...
        self.play(Create(tch1), Create(tch2), FadeIn(step_texts[3], shift=RIGHT*0.2), 
                  run_time=1.0, rate_func=rate_functions.ease_out_cubic)
        self.wait(2.0)

        # Step 5 - Finale Subtle Dashed Box
        subtle_box1 = SurroundingRectangle(caller, color=DIM_GREY, corner_radius=0.15, stroke_width=2, buff=0.3)
        subtle_box2 = SurroundingRectangle(receiver, color=DIM_GREY, corner_radius=0.15, stroke_width=2, buff=0.3)
        ...
        # ── Callout Summary ──
        note = callout_box("MSC coordinates paging + location tracking via VLR", ACCENT_GRN)
