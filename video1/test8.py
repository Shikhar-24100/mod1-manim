...
class AssumptionsVisualization(Scene):
    ...
    def construct(self):
        ...
        subtitle = Text("What we assume vs What we get", font_size=28, color=BLUE)
        ...
        # ========== IDEAL SCENARIO ==========
        ideal_label = Text("IDEAL SCENARIO", font_size=36, color=GREEN, weight=BOLD)
        ...
        rx_tower.shift(RIGHT * 4 + DOWN * 0.5)
        ...
        transition_text = Text("But in the REAL WORLD...", font_size=36, color=RED, weight=BOLD)
        ...
        # Real world label
        real_label = Text("REAL WORLD", font_size=36, color=RED, weight=BOLD)
        ...
        trees = VGroup()
        for pos in [LEFT * 3.2, LEFT * 0.5, RIGHT * 4]:
            tree = VGroup(
        ...
        # Person
        person = VGroup(
            Circle(radius=0.15, color="#FFD700", fill_opacity=1),  # Head
            Line(ORIGIN, DOWN * 0.5, color="#FFD700", stroke_width=6),  # Body
            Line(ORIGIN, DL * 0.3, color="#FFD700", stroke_width=4),  # Leg 1
            Line(ORIGIN, DR * 0.3, color="#FFD700", stroke_width=4)   # Leg 2
        )
        ...
        car = VGroup(
            ...
            Circle(radius=0.1, color=BLACK, fill_opacity=1).shift(LEFT * 0.2 + DOWN * 0.2),
            Circle(radius=0.1, color=BLACK, fill_opacity=1).shift(RIGHT * 0.2 + DOWN * 0.2)
        )
        ...
        warning = VGroup(
            ...
            Text("Only valid under ideal conditions!", font_size=22, color=RED)
...
class LimitationScenarios(Scene):
    ...
    def construct(self):
        # Title
        title2 = Text("Friis Equation Limitations", font_size=44, weight=BOLD)
        subtitle = Text("Prediction vs Reality", font_size=28, color=WHITE)
        ...
        self.play(FadeIn(subtitle, shift=UP))
        self.play(FadeOut(subtitle), FadeOut(title2))

        title = Text("Changing Surrounding Conditions", font_size=36)
        self.play(Write(title))
        self.wait(1)
        ...
        friis_eq.to_corner(UL, buff=0.6).shift(DOWN * 0.4)
        ...
        surroundingrect1 = SurroundingRectangle(prediction_box, color=GREEN, stroke_width=2, buff=0.2)
        # prediction_box.add_to_back(surroundingrect1)
        
        self.play(FadeIn(prediction_box, shift=LEFT))
        self.play(Create(surroundingrect1))
        self.wait(1)
        ...
        vehicle = VGroup(
            ...
            Circle(radius=0.11, color=GREY, fill_opacity=1).shift(LEFT * 0.28 + DOWN * 0.22),
            Circle(radius=0.11, color=GREY, fill_opacity=1).shift(RIGHT * 0.28 + DOWN * 0.22)
        )
        ...
        # ========== SHOW MULTIPATH ==========
        multipath_label = Text("Multipath Propagation:", font_size=22, color=WHITE)
        ...
        actual_box.shift(DOWN * 1)
        surroundingrect2 = SurroundingRectangle(actual_box, color=RED, stroke_width=2, buff=0.2)
        self.play(FadeIn(actual_box, shift=UP))
        self.play(Create(surroundingrect2))
        self.wait(1)
        # Show mismatch
        mismatch = VGroup(
        ...
        self.wait(8)
        self.play(FadeOut(mismatch), FadeOut(error_arrow), FadeOut(actual_box), FadeOut(surroundingrect2), FadeOut(prediction_box), FadeOut(surroundingrect1), FadeOut(paths), FadeOut(path_labels), FadeOut(multipath_label), FadeOut(scenario_label), FadeOut(ground), FadeOut(tx), FadeOut(rx), FadeOut(tx_label), FadeOut(rx_label), FadeOut(buildings), FadeOut(blocked_regions), FadeOut(vehicle))
        # ========== CONCLUSION ==========
        self.play(FadeOut(friis_eq), FadeOut(friis_label))
        conclusion = VGroup(
