...
class SignalEffectsSummary(Scene):
    ...
    def construct(self):
        ...
        summary_points = VGroup(
            VGroup(
                Text("1. Path Loss:", font_size=24, color=WHITE, weight=BOLD),
                Text("   Signal decreases with distance", font_size=20, color=WHITE, slant=ITALIC)
            ).arrange(RIGHT, buff=0.15, aligned_edge=UP),
            
            VGroup(
                Text("2. Shadowing:", font_size=24, color=WHITE, weight=BOLD),
                Text("   Random variation due to obstacles", font_size=20, color=WHITE, slant=ITALIC)
            ).arrange(RIGHT, buff=0.15, aligned_edge=UP),
            
            VGroup(
                Text("3. Multipath:", font_size=24, color=YELLOW, weight=BOLD),
                Text("   Multiple signal paths", font_size=20, color=YELLOW, slant=ITALIC)
            ).arrange(RIGHT, buff=0.15, aligned_edge=UP)
