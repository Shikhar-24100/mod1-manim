...
class WirelessSignalScatteringWithSVG(Scene):
    def construct(self):
        ...
        tower.move_to(LEFT * 5 + DOWN * 2)
        
        rough_wall = SVGMobject("rough.svg", height=2)
        rough_wall.move_to(ORIGIN)
        
        mobile = SVGMobject("mobile.svg", height=1.5)
        mobile.move_to(RIGHT * 5 + DOWN * 2)
        
        # Add labels
        tower_label = Text("TX (Transmitter)", font_size=16, color=BLUE).next_to(tower, DOWN, buff=0.3)
        wall_label = Text("Rough Surface", font_size=16, color=GREY_A).next_to(rough_wall, UP, buff=0.5)
        mobile_label = Text("RX (Receiver)", font_size=16, color=GREEN).next_to(mobile, DOWN, buff=0.3)
        
        # Add SVG elements
        self.play(FadeIn(tower), FadeIn(mobile), FadeIn(rough_wall), run_time=1.5)
        self.play(Write(tower_label), Write(mobile_label), Write(wall_label))
        ...
        incident_ray_start = tower.get_right() + UP * 0.3
        incident_ray_end = rough_wall.get_left() + UP * 0.3
        ...
        receiver_ray_start = scatter_point + np.array([0.5, -1.5, 0])
        receiver_ray_end = mobile.get_left()
        ...
        explanation = VGroup(
            Text("Key Points:", font_size=16, color=WHITE, weight=BOLD),
            Text("1. Rough surface size ~ wavelength → Effective scattering", font_size=12, color=YELLOW),
            Text("2. Incident wave interacts with surface irregularities", font_size=12, color=BLUE),
            Text("3. Signal scattered in multiple directions (diffuse scattering)", font_size=12, color=ORANGE),
            Text("4. Receiver captures one scattered component (multipath propagation)", font_size=12, color=GREEN),
        )
